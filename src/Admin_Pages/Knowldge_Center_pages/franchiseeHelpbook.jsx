...
function franchiseeHelpbook() {

<<<<<<< HEAD
  const [file,setFile]= useState(null); 
  const [fileDescription , SetFileDescription] = useState("");
  const [helpbookData , setHelpbookData] = useState([]);
...
<<<<<<< HEAD
  ...
  const category = "franchiseeHelpbook";
              ...
              <div className="statbox widget">
                  ...
                  <div className="widget-header bg-light">
                    <div className="d-flex align-items-center justify-content-between ">
                        <h4 className="p-3" >Franchisee Helpbook</h4>
...
=======
    try {
      const response = await apiPost(config.deleteKnowledgeCenterFile, { "file_id": file_id });
      //  await Swal.fire("Deleted!", "File has been deleted.", "success");
      if (response.status === 200) {
        toast.success("File deleted successfully.");
        fetchTabledata(); // Refresh the table
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await apiFormDataPost(config.uploadKnowledgeCenterFile, formData);
...
export default franchiseeHelpbook;
